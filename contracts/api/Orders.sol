...
contract Orders is Roles {
    ...
    function _submitOrder(
        ...
        require(params.margin >= asset.minMargin,"!min margin");
        require(params.margin <= asset.maxMargin,"!max margin");
        ...
        require(
            !riskStore.isAddressBannedForMarket(user, params.market),
            "!banned-user-market"
        );
        ...
        if (params.expiry > 0) {
            ...
            require(
                (params.orderType == 0 &&
                    ttl <= orderStore.maxMarketOrderTTL()) ||
        ...
        if (params.cancelOrderId > 0) {
            require(
                orderStore.isUserOrder(params.cancelOrderId, user),
                "!user-oco"
            );
        }
        ...
        uint256 feeWithRebate = (originalFee *
            (BPS_DIVIDER - feeRebateBps - referralRebateBps)) / BPS_DIVIDER;
    ...
    function cancelOrder(uint256 orderId) public ifNotPaused {
    ...
    function _getPythPrice(
        ...
        uint256 baseConversion = 10 **
            uint256(int256(18) + retrievedPrice.expo);
    ...
    function getChainlinkPrice(address feed) public view returns (uint256) {
        return chainlink.getPrice(feed);
    }
