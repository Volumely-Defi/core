...
contract Positions is Roles {
    ...
    function increasePosition(uint256 orderId, uint256 price) public onlyContract{
        ...
        PositionStore.Position memory position = positionStore.get(
            order.user,
            order.asset,
            order.market
        );
        ...
        if (position.size == 0) {
            ...
            position.fundingTracker = fundingStore.getFundingTracker(
                order.asset,
                order.market
            );
        }
    ...
    function _payReferrer(
        ...
        if (referrer != address(0)) {
            uint256 referrerShareBps = referralStore.getReferrerShareForUser(
                user
            );
